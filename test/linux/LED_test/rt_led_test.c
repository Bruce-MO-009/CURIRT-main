...
void ecatcheck_task(void* cookie)
{
    rt_task_set_periodic(NULL, TM_NOW, 50000);
    for (;;) {
       ecatcheck(NULL);
...
{   
    ...
    int i = 1, cnt = 512;
    float t_max = 0, t_min = 1000000, t_sum = 0, t0, t1;
    uint8_t addr = 0;
   
    rt_task_set_periodic(NULL, TM_INFINITE, 5000000);
    ...
    while (i < cnt) {
        if (inOP) {
            ...
            set_output_int8(1, addr, (char)(i % 256));
            ec_send_processdata();
            ec_receive_processdata(EC_TIMEOUTRET);
    ...
    printf("PDO max time %f, min time %f, ave time %f us\n", t_max, t_min, t_sum / cnt);
    ethercat_close();
    rt_task_delete(&task_led);
    return;
}
...
int main(int argc, char *argv[])
{
   ...
   if (argc > 1)
   {
      ...
      /*RT_TASK task_check;
      err = rt_task_create(&task_check, "Check", 0, 50, 0);
      if (!err)
          rt_task_start(&task_check, &ecatcheck_task, NULL);*/
      
      mlockall(MCL_CURRENT|MCL_FUTURE);
      err = rt_task_create(&task_led, "Led", 0, 50, 0);
      if (!err)
          rt_task_start(&task_led, &led_task, NULL);
