...
//          [0x001B.0] 0x0000:0x00 0x08
    ...
    wsum += set_pdo_write32(slave, 0x1607, 5, 0x60600008); // mode of operation
    wsum += set_pdo_write32(slave, 0x1607, 6, 0x00000008); // mode of operation
    ...
    wsum += set_pdo_write32(slave, 0x1A07, 5, 0x60610008); // mode of operation display
    wsum += set_pdo_write32(slave, 0x1A07, 6, 0x00000008); // mode of operation display
    //
...
uint16 ec_set_state_machine(uint16 reqstate, int timeout)
{
   ...
   do {
      ec_send_processdata();
      ec_receive_processdata(EC_TIMEOUTRET);
      ec_statecheck(0, reqstate, timeout * 10);
...
void motor_control(char *ifname)
{
   ...
   ec_set_state_machine(EC_STATE_PRE_OP, EC_TIMEOUTSTATE);
