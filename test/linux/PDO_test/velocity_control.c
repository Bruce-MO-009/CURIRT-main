...
//          [0x001B.0] 0x0000:0x00 0x08
    ...
    wsum += set_pdo_write8 (slave, 0x6060, 0, 9); // Set mode of operation to CST
...
// Motor State: Enable
        ...
        for (i = 0; i < 1000; ++ i) {
            ...
            set_output_int16(1, 0x0A >> 1, ControlWord);
            set_output_int32(1, 0x04 >> 1, i * 500);
            ec_send_processdata();
            wkc = ec_receive_processdata(EC_TIMEOUTRET);
            get_input_int32(1, 0x00 >> 1, &JointCurPos);
        ...
        for (i = 0; i < 1000; ++ i) {
            set_output_int16(1, 0x0A >> 1, ControlWord);
            set_output_int32(1, 0x04 >> 1, (1000-i) * 500);
            ec_send_processdata();
            wkc = ec_receive_processdata(EC_TIMEOUTRET);
            get_input_int32(1, 0x00 >> 1, &JointCurPos);
            get_input_int32(1, 0x04 >> 1, &JointCurVel);
        ...
        for (i = 0; i < 1000; ++ i) {
            set_output_int16(1, 0x0A >> 1, ControlWord);
            set_output_int32(1, 0x04 >> 1, i * 1000);
            ec_send_processdata();
            wkc = ec_receive_processdata(EC_TIMEOUTRET);
            get_input_int32(1, 0x00 >> 1, &JointCurPos);
            get_input_int32(1, 0x04 >> 1, &JointCurVel);
        ...
        for (i = 0; i < 1000; ++ i) {
            ...
            set_output_int16(1, 0x0A >> 1, ControlWord);
            set_output_int32(1, 0x04 >> 1, (1000 - i) * 1000);
            ec_send_processdata();
            wkc = ec_receive_processdata(EC_TIMEOUTRET);
            get_input_int32(1, 0x00 >> 1, &JointCurPos);
            get_input_int32(1, 0x04 >> 1, &JointCurVel);
...
//      pthread_create( &thread1, NULL, (void *) &ecatcheck, (void*) &ctime);
      ...
      err = rt_task_create(&task_velocity, "velocity", 0, 50, 0);
      if (!err)
          rt_task_start(&task_velocity, &velocity_task, NULL);
      if (ethercat_initial(argv[1])) {
