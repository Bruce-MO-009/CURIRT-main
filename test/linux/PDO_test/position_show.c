...
//          [0x001B.0] 0x0000:0x00 0x08
    ...
    wsum += set_pdo_write8 (slave, 0x6060, 0, 8); // Set mode of operation to CST

    wsum += set_pdo_write8 (slave, 0x1C12, 0, 0);
    wsum += set_pdo_write8 (slave, 0x1601, 0, 0);
    wsum += set_pdo_write32(slave, 0x1601, 1, 0x60400010); // Control word
    wsum += set_pdo_write32(slave, 0x1601, 2, 0x607A0020); // Target position
    wsum += set_pdo_write32(slave, 0x1601, 3, 0x60B10020); // Velocity offset
    // wsum += set_pdo_write16(slave, 0x1603, 4, 0x60710010); // Target torque
    wsum += set_pdo_write32(slave, 0x1601, 4, 0x60B20010); // torque offset
    wsum += set_pdo_write32(slave, 0x1601, 5, 0x60600008); // mode of operation
    wsum += set_pdo_write32(slave, 0x1601, 6, 0x00000008); // none
    wsum += set_pdo_write32(slave, 0x1601, 0, 6);
    wsum += set_pdo_write16(slave, 0x1C12, 1, 0x1601);
    wsum += set_pdo_write8 (slave, 0x1C12, 0, 1);


    wsum += set_pdo_write8 (slave, 0x1C13, 0, 0);
    wsum += set_pdo_write8 (slave, 0x1A03, 0, 0);
    wsum += set_pdo_write32(slave, 0x1A03, 1, 0x60410010); // Status word
    wsum += set_pdo_write32(slave, 0x1A03, 2, 0x60640020); // Position actual value
    wsum += set_pdo_write32(slave, 0x1A03, 3, 0x606C0020); // Velocity actual value
    wsum += set_pdo_write32(slave, 0x1A03, 4, 0x60770010); // torque offset
    wsum += set_pdo_write32(slave, 0x1A03, 5, 0x60610008); // mode of operation display
    wsum += set_pdo_write32(slave, 0x1A03, 6, 0x00000008); // none
    //
    wsum += set_pdo_write8 (slave, 0x1A03, 0, 6);
    wsum += set_pdo_write16(slave, 0x1C13, 1, 0x1A03);
    ...
    if (ec_init(ifname)) {
        ...
        if ( ec_config_init(FALSE) > 0 ) {
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL )
            {
                ...
                set_output_int16(1, 0x00 >> 1, ControlWord);

                ec_send_processdata();
                wkc = ec_receive_processdata(EC_TIMEOUTRET);

                get_input_int16 (1, (0x0E - 0x0E) >> 1, (int16_t *)&StateWord);
                get_input_int32 (1, (0x10 - 0x0E) >> 1, &JointCurPos);
                get_input_int8  (1, (0x1A - 0x0E) >> 1, &Mode);
                ...
                ControlWord = 7;
                set_output_int16(1, (0x00 - 0x00) >> 1, ControlWord);
                set_output_int32(1, (0x02 - 0x00) >> 1, JointCurPos);
                ec_send_processdata();
                wkc = ec_receive_processdata(EC_TIMEOUTRET);
...
//      pthread_create( &thread1, NULL, (void *) &ecatcheck, (void*) &ctime);
      ...
      if (ethercat_initial(argv[1])) {
        ...
        for (i = 0; i < 1000; ++ i) {
            ...
            if (i % 100 == 0)
                printf("State: %d, CurPos: %d\n", StateWord, JointCurPos);
